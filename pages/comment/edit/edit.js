...
function hasBadWordCallback(app, that, message, res) {
  console.log("hasBadWordCallback");
  console.log(res);
  if (res.code == 0) {
...
function sendMessage(that, message) {
  ...
  app.doLogin(function () {
    ...
    uploadImages(message.images, function (images) {
      ...
      data.address = message.address.name;
      data.content = app.util.formatContentForServer(message.content);
      wx.request({
        url: app.globalData.url.api.addNote + "?token=" + app.globalData.userToken + "&imageUrls=" + JSON.stringify(images) + "&latitude=" + message.address.latitude + "&longitude=" + message.address.longitude + "&wxapp=1&fdPartnerID=" + data.fdPartnerID + "&pos=" + data.pos + "&key=" + data.key + "&sign=" + data.sign,
        method: "POST",
        data: data,
        header: {
        ...
        success: function (res) {
          ...
          if (res.data.errcode == 0) {
            ...
            rawData.address = message.address.name;
            rawData.rawMeter = parseInt(app.util.getPonitToPointDistance(
              message.address.latitude,
              message.address.longitude,
              app.globalData.location.latitude,
              app.globalData.location.longitude
            ));
            rawData.meter = app.util.formatDistance(rawData.rawMeter);
            console.log("rawData");
            console.log(rawData);
    ...
    BMap.regeocoding({
      ...
      success: function (res) {
        ...
        if (res.originalData && res.originalData.result) {
          ...
          if (result.location) {
            message.address.latitude = result.location.lat;
            message.address.longitude = result.location.lng;
          }
          that.setData({
            message: message
          });
